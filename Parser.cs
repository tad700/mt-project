...
//		}
        ...
        public bool IsVarDecl()
        {
            ...
            LocalBuilder local = emit.AddLocalVar(name.value, type);
            symbolTable.AddLocalVar(name, local);

            // Проверка за инициализация (= Expression)
            if (CheckSpecialSymbol("="))
            {
                Type exprType;
                if (!IsExpression(null, out exprType)) Error("Очаквам израз");
                if (!AssignableTypes(type, exprType))
                    Error($"Несъвместими типове: {type.Name} и {exprType.Name}");

                // Emit: Запиши стойността в променливата
                emit.AddLocalVarAssigment(local);
            }

            if (!CheckSpecialSymbol(";")) Error("Очаквам ';'");
            return true;
        }
